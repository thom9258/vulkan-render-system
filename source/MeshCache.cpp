...
SimpleMeshRef::SimpleMeshRef(std::uint64_t id)
	: m_id{id}
{
}

std::uint64_t SimpleMeshRef::id() const
{
	return m_id;
}
...
AnimatedMeshRef::AnimatedMeshRef(std::uint64_t id)
	: m_id{id}
{
}

std::uint64_t AnimatedMeshRef::id() const
{
	return m_id;
}
...
auto MeshCache::get(SimpleMeshRef ref) 
	-> TexturedMesh*
{
 	auto found = m_mesh_cache.find(ref);
	if (found == m_mesh_cache.end()) {
	...
	-> AnimatedMeshRef
{
	...
	if (found != m_animated_mesh_cache.end()) {
		throw std::runtime_error("fatal! animated texture ref could not be generated!");
	}
...
auto MeshCache::get(AnimatedMeshRef ref) 
	-> AnimatedMesh*
{
 	auto found = m_animated_mesh_cache.find(ref);
	if (found == m_animated_mesh_cache.end()) {
