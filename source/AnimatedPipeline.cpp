...
AnimatedPipeline::AnimatedPipeline(
  ...
  std::string const vertexshader_name = "AnimatedMaterial.vert.spv";
  std::string const fragmentshader_name = "AnimatedMaterial.frag.spv";

  logger.info(std::source_location::current(),
              std::format("Creating Pipeline {}", pipeline_name));
  logger.info(std::source_location::current(),
              std::format("  Vertex Shader {}", vertexshader_name));
  logger.info(std::source_location::current(),
  ...
  const auto fragment_path =
      FragmentPath{shader_root_path / fragmentshader_name};
  auto shaderstage_infos = create_shaderstage_infos(context->device.get(),
                                                    vertex_path, fragment_path);

  if (!shaderstage_infos) {
    std::string const msg =
        std::format("{} could not load vertex/fragment sources {} / {}",
                    pipeline_name, vertexshader_name, fragmentshader_name);
  ...
  const auto bindingDescriptions =
      binding_descriptions(VertexAnimatedPosNormColorUV{});
  const auto attributeDescriptions =
      attribute_descriptions(VertexAnimatedPosNormColorUV{});
  ...
  m_global_set_layout = context->device.get().createDescriptorSetLayoutUnique(
      frame_uniform_setinfo, nullptr);

  logger.info(std::source_location::current(), "Created frame uniform layout");

  std::array<vk::DescriptorSetLayoutBinding, 1> model_info_uniform_bindings{
      vk::DescriptorSetLayoutBinding{}
          .setStageFlags(vk::ShaderStageFlagBits::eVertex)
          .setBinding(0)
          .setDescriptorCount(1)
  ...
  auto ambient_texture_setinfo =
      vk::DescriptorSetLayoutCreateInfo{}
          .setFlags(vk::DescriptorSetLayoutCreateFlags())
          .setBindings(ambient_texture_bindings);

  m_ambient.layout = context->device.get().createDescriptorSetLayoutUnique(
      ambient_texture_setinfo, nullptr);
  ...
  auto diffuse_texture_setinfo =
      vk::DescriptorSetLayoutCreateInfo{}
          .setFlags(vk::DescriptorSetLayoutCreateFlags())
          .setBindings(diffuse_texture_bindings);

  m_diffuse.layout = context->device.get().createDescriptorSetLayoutUnique(
      diffuse_texture_setinfo, nullptr);
  ...
  auto normal_texture_setinfo =
      vk::DescriptorSetLayoutCreateInfo{}
          .setFlags(vk::DescriptorSetLayoutCreateFlags())
          .setBindings(normal_texture_bindings);

  m_normal.layout = context->device.get().createDescriptorSetLayoutUnique(
      normal_texture_setinfo, nullptr);
  ...
  auto specular_texture_setinfo =
      vk::DescriptorSetLayoutCreateInfo{}
          .setFlags(vk::DescriptorSetLayoutCreateFlags())
          .setBindings(specular_texture_bindings);

  m_specular.layout = context->device.get().createDescriptorSetLayoutUnique(
      specular_texture_setinfo, nullptr);

  logger.info(std::source_location::current(),
              "Created texture descriptorset layouts");
  ...
  auto directional_shadowmap_setinfo =
      vk::DescriptorSetLayoutCreateInfo{}
          .setFlags(vk::DescriptorSetLayoutCreateFlags())
          .setBindings(directional_shadowmap_bindings);

  m_directional_shadowmap_layout =
      context->device.get().createDescriptorSetLayoutUnique(
          directional_shadowmap_setinfo, nullptr);
  ...
  m_spot_shadowmap_layout =
      context->device.get().createDescriptorSetLayoutUnique(
          spot_shadowmap_setinfo, nullptr);
  ...
  Pixel8bitRGBA constexpr ambient_basic{ambient_color, ambient_color,
                                        ambient_color, 255};
  ...
  const auto frame_uniform_allocate_info =
  ...
  // NOTE: due to only having 1 identical layout for each set, we need to
  // allocate
  ...
  for (auto &uniform : m_global_set_uniforms) {
    ...
    uniform.directional_shadowcaster.write(context->device.get(),
  ...
  //       initialize them before use. Aftwerwards, it is not nessecary to
  //       update with a write
  ...
  for (size_t i = 0; i < m_global_set_uniforms.size(); i++) {
    std::array<vk::WriteDescriptorSet, 8> writes{
        ...
        vk::WriteDescriptorSet{}
            ...
            .setDescriptorType(vk::DescriptorType::eUniformBuffer)
            .setBufferInfo(
                m_global_set_uniforms[i].directionallight.buffer_info()),
        ...
        vk::WriteDescriptorSet{}
            ...
            .setDescriptorType(vk::DescriptorType::eUniformBuffer)
            .setBufferInfo(
                m_global_set_uniforms[i].lightarray_lengths.buffer_info()),
        ...
        vk::WriteDescriptorSet{}
            ...
            .setDescriptorCount(1)
            .setDescriptorType(vk::DescriptorType::eUniformBuffer)
            .setBufferInfo(m_global_set_uniforms[i]
                               .directional_shadowcaster.buffer_info()),
...
AnimatedPipeline::AnimatedPipeline(AnimatedPipeline &&rhs) noexcept {
...
AnimatedPipeline &AnimatedPipeline::operator=(AnimatedPipeline &&rhs) noexcept {
  std::swap(m_layout, rhs.m_layout);
  std::swap(m_pipeline, rhs.m_pipeline);
  std::swap(m_global_set_layout, rhs.m_global_set_layout);
  std::swap(m_global_set_uniforms, rhs.m_global_set_uniforms);
...
void AnimatedPipeline::render(
  ...
  if (!m_ambient.sets.contains(&m_ambient.default_texture)) {
    m_ambient.sets.insert({&m_ambient.default_texture,
                           create_texture_descriptorset(
                               device, m_ambient.layout.get(), descriptor_pool,
                               m_ambient.default_texture)});
  ...
  if (!m_diffuse.sets.contains(&m_diffuse.default_texture)) {
    m_diffuse.sets.insert({&m_diffuse.default_texture,
                           create_texture_descriptorset(
                               device, m_diffuse.layout.get(), descriptor_pool,
                               m_diffuse.default_texture)});
  ...
  if (!m_specular.sets.contains(&m_specular.default_texture)) {
    m_specular.sets.insert({&m_specular.default_texture,
                            create_texture_descriptorset(
                                device, m_specular.layout.get(),
                                descriptor_pool, m_specular.default_texture)});
  ...
  if (!m_normal.sets.contains(&m_normal.default_texture)) {
    m_normal.sets.insert({&m_normal.default_texture,
                          create_texture_descriptorset(
                              device, m_normal.layout.get(), descriptor_pool,
                              m_normal.default_texture)});
  ...
  std::ranges::for_each(lights,
                        std::bind_front(sort_light, &logger, &sorted_lights));
  ...
  if (!sorted_lights.points.empty()) {
    ...
    m_global_set_uniforms[*current_flightframe].pointlight.write(
        device, data.data(), data.size());
  ...
  if (!sorted_lights.spots.empty()) {
    ...
    m_global_set_uniforms[*current_flightframe].spotlight.write(
        device, data.data(), data.size());
  ...
  if (!sorted_lights.directionals.empty()) {
    ...
    m_global_set_uniforms[*current_flightframe].directionallight.write(
        device, data.data(), data.size());
    lightarray_lengths_data.directional_length = length;
  }

  m_global_set_uniforms[*current_flightframe].lightarray_lengths.write(
      device, &lightarray_lengths_data, lightarray_lengths_count);

  if (shadowcasters.directional.caster.has_value()) {
    DirectionalShadowCasterUniformData data(
        shadowcasters.directional.caster.value());
    m_global_set_uniforms[*current_flightframe].directional_shadowcaster.write(
        device, &data, directional_shadowcasters_count);
  } else {
    DirectionalShadowCasterUniformData data{};
    data.exists = false;
    m_global_set_uniforms[*current_flightframe].directional_shadowcaster.write(
  ...
  if (shadowcasters.spot.caster.has_value()) {
    ...
    m_global_set_uniforms[*current_flightframe].spot_shadowcaster.write(
        device, &data, spot_shadowcasters_count);
  } else {
  ...
  for (AnimatedRenderable &renderable : renderables) {
    ...
    auto try_get_texture_from_cache =
        [](TextureSamplerCache &texturesampler_cache,
           std::optional<TextureSamplerRef> ref) -> TextureSamplerReadOnly * {
      if (ref.has_value()) {
    ...
    TextureSamplerReadOnly *ambient_texture =
        try_get_texture_from_cache(texturesampler_cache, renderable.ambient);
    ...
    if (ambient_texture != last_ambient_texture) {
      if (!m_ambient.sets.contains(ambient_texture)) {
        m_ambient.sets.insert(
            {ambient_texture,
             create_texture_descriptorset(device, m_ambient.layout.get(),
                                          descriptor_pool, *ambient_texture)});
      ...
      commandbuffer.bindDescriptorSets(vk::PipelineBindPoint::eGraphics,
                                       m_layout.get(), *m_ambient.set_index,
                                       descriptorset.size(),
                                       descriptorset.data(), 0, nullptr);
      last_ambient_texture = ambient_texture;
    }

    TextureSamplerReadOnly *diffuse_texture =
        try_get_texture_from_cache(texturesampler_cache, renderable.diffuse);
    ...
    if (diffuse_texture != last_diffuse_texture) {
      if (!m_diffuse.sets.contains(diffuse_texture)) {
        m_diffuse.sets.insert(
            {diffuse_texture,
             create_texture_descriptorset(device, m_diffuse.layout.get(),
                                          descriptor_pool, *diffuse_texture)});
      ...
      commandbuffer.bindDescriptorSets(vk::PipelineBindPoint::eGraphics,
                                       m_layout.get(), *m_diffuse.set_index,
                                       descriptorset.size(),
                                       descriptorset.data(), 0, nullptr);
      last_diffuse_texture = diffuse_texture;
    }

    TextureSamplerReadOnly *specular_texture =
        try_get_texture_from_cache(texturesampler_cache, renderable.specular);
    ...
    if (specular_texture != last_specular_texture) {
      if (!m_specular.sets.contains(specular_texture)) {
        m_specular.sets.insert(
            {specular_texture,
             create_texture_descriptorset(device, m_specular.layout.get(),
                                          descriptor_pool, *specular_texture)});
      ...
      commandbuffer.bindDescriptorSets(vk::PipelineBindPoint::eGraphics,
                                       m_layout.get(), *m_specular.set_index,
                                       descriptorset.size(),
                                       descriptorset.data(), 0, nullptr);
      last_specular_texture = specular_texture;
    }

    TextureSamplerReadOnly *normal_texture =
        try_get_texture_from_cache(texturesampler_cache, renderable.normal);
    if (!normal_texture) {
    ...
    if (normal_texture != last_normal_texture) {
...

      {
        ...
        commandbuffer.bindDescriptorSets(vk::PipelineBindPoint::eGraphics,
                                         m_layout.get(), *m_normal.set_index,
                                         descriptorset.size(),
                                         descriptorset.data(), 0, nullptr);
    ...
    AnimatedMesh *mesh = mesh_cache.get(renderable.mesh.value());
    ...
    commandbuffer.draw(vertices_length, instanceCount, firstVertex,
                       firstInstance);
  }
}
