...
const std::string resources_root = "../../compiled_shaders/";
...
int main()
{
	PresentationContext context(2);

	const auto window = context.get_window_extent();
	const auto scale = static_cast<float>(window.width) / static_cast<float>(window.height);
	...
	Pipelines pipelines = create_pipelines(context.physical_device,
										   context.device.get(),
										   context.command_pool(),
										   context.graphics_queue(),
										   descriptor_pool.get(),
	...
	auto loaded_statue =
		load_bitmap(assets_root + "textures/texture.jpg", BitmapPixelFormat::RGBA);
	...
	auto loaded_lulu = load_bitmap(assets_root + "textures/lulu.jpg", BitmapPixelFormat::RGBA);
    ...
    world_info.projection = glm::perspective(glm::radians(70.f), scale, 0.1f, 200.0f);
