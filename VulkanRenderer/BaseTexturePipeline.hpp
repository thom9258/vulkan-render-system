								 ...
								 TextureSamplerReadOnly& texture)
{
	...
	for (uint32_t i = 0; i < frames_in_flight; i++) {
		...
		device.updateDescriptorSets(write.size(), write.data(), 0, nullptr);
	}
...
BaseTexturePipeline
create_base_texture_pipeline(vk::PhysicalDevice& physical_device,
							 vk::Device& device,
...
{
	const std::string pipeline_name = "Texture";
	...
	pipeline.camera_descriptor.layout =
		device.createDescriptorSetLayoutUnique(camera_set_info, nullptr);
	...
	pipeline.texture_descriptor.layout =
		device.createDescriptorSetLayoutUnique(base_texture_set_info, nullptr);
	...
	for (uint32_t i = 0; i < frames_in_flight; i++) {
		...
		device.updateDescriptorSets(camera_write.size(), camera_write.data(), 0, nullptr);
	}
	if (debug_print)
	...
	pipeline.base_texture = 
		...
		| draw_checkerboard(yellow, CheckerSquareSize{4})
		| move_canvas_to_gpu(physical_device,
							 device,
							 command_pool,
							 graphics_queue,
